...
if (!process.env.SESSION_SECRET) {
    console.warn('ATENCAO: SESSION_SECRET nao configurado. Usando um secret gerado automaticamente.');
    console.warn('Para producao, configure a variavel de ambiente SESSION_SECRET.');
}

const JWT_SECRET = process.env.JWT_SECRET || crypto.randomBytes(64).toString('hex');
if (!process.env.JWT_SECRET) {
    console.warn('ATENCAO: JWT_SECRET nao configurado. Usando um secret gerado automaticamente.');
    console.warn('Para producao, configure a variavel de ambiente JWT_SECRET.');
}
...
if (DEV_TEMP_ACCESS_ENABLED) {
    console.log('Acesso temporario de desenvolvimento HABILITADO');
    console.warn('ATENCAO: Desabilite DEV_TEMP_ACCESS antes de fazer deploy em producao!');
} else {
                ...
                if (passwordMatch) {
                    ...
                    db.run('UPDATE usuarios SET password = ?, password_hashed = 1 WHERE id = ?', 
                        ...
                        (err) => {
                            ...
                            else console.log(`Senha do usuario ${username} migrada para bcrypt`);
                        }
                    );
                }
            }
...
function iniciarServidor() {
    ...
    app.listen(PORT, '0.0.0.0', async () => {
        ...
        console.log('Google Drive desativado. Sistema operando apenas com armazenamento local.');
    });
}
